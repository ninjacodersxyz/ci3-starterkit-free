...

{
	 ...
	 * @return  TRUE on success, FALSE otherwise
	 */
	function add_js_array($scripts, $path=NULL, $type = 'import', $defer = FALSE)
	{
		$success = TRUE;
		$js = NULL;
		$this->CI->load->helper('url');
		foreach($scripts as $key => $script) {
			$filepath = asset_url() . (($path)? $path : '') . $script;
	 ...
	 * @return  TRUE on success, FALSE otherwise
	 */
	function add_css_array($styles, $path=NULL, $type = 'link', $media = FALSE)
	{
		$this->CI->load->helper('url');
		foreach($styles as $value) {
			$success = TRUE;
			$filepath = asset_url() . (($path)? $path : '') . $value;
			$css = '<link type="text/css" rel="stylesheet" href="' . $filepath . '" />';
